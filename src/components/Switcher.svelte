<script>
  ...
  function onChange(switcherType, type, changedData) {
    const eventType = switcherType === 'date' ? 'dateChange' : 'timeChange';
    dispatch(eventType, {
      type,
      changedData,
    });
  ...
  let onMouseUp = () => {
    ...
    setPosition();
    onChange(switcherType, type, -finalPosition / 50);
  };
</script>
...
<div
  ...
  <ul bind:this={itemWrapper} class="switcher-container">
    {#each data as item}
      <li>{item}</li>
    {/each}
  </ul>
