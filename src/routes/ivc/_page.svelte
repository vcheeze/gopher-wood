<script lang="ts">
	...
	import { Spinner, Tabs, TabItem, Label, Input, Select, Modal, Button } from 'flowbite-svelte';
	import type { SelectOptionType } from 'flowbite-svelte/types';
	import { t } from '$lib/translations';
	// import { Spinner, Tabs, Dialog } from '$lib/components';
	...
	let showDialog = false,
		...
		dialogBody = '';
	const tabItems = [
		{ id: 'newAppointment', label: 'ivc.options.newAppointment' },
		{ id: 'modifyAppointment', label: 'ivc.options.modifyAppointment' }
	];
	...
	const today = dayjs();
	let dateOptions: SelectOptionType[] = [];
	let timeOptions: SelectOptionType[] = [];
	let prevAppointmentOptions: SelectOptionType[] = []; // previous appointments
	let fullName = '',
		mobile = '',
	...
	async function updateTimeslots() {
		...
		const now = dayjs();
		let response = await fetch(`/api/getBookedSlots?date=${date.format('YYYY-MM-DD')}`);
		if (response.ok) {
	...
	function addTimeslots(date, isMorning, bookedSlots) {
		...
		while (fromTime <= toTime) {
			if (slotsForPeriod.length < 1 || fromTime.format('HH:mm:ss') !== slotsForPeriod[0].time) {
				options.push({
	...
	function updateFieldsState() {
		...
		hidePrevAppointments = !prevAppointmentsFetched || selectedTab === 'newAppointment';
		hideNewOptions =
			!prevAppointmentsFetched || selectedEditType === 'cancel' || selectedTab === 'newAppointment';
	}

	function updateButtonsState() {
		if (selectedTab === 'newAppointment') {
			disableSubmitBtn = !fullName || !mobile || !selectedDate || !selectedTime;
		} else {
			// search button disable logic
			disableSearchBtn = !fullName || !mobile;
			if (selectedEditType === 'update') {
				disableSubmitBtn =
					!fullName || !mobile || !selectedPrevAppointment || !selectedDate || !selectedTime;
			} else {
				disableSubmitBtn = !fullName || !mobile || !selectedPrevAppointment;
			}
		}
	}
	...
	async function onSubmit(e) {
		...
		if (response.ok) {
			...
			dialogTitle = $t('ivc.dialog.success.title');
			dialogBody = $t('ivc.dialog.success.description');
		} else {
			dialogStatus = 'error';
			dialogTitle = $t('ivc.dialog.error.title');
			dialogBody = $t('ivc.dialog.error.description');
		}
	...
	async function onSearch(e) {
		...
		if (response.ok) {
			...
			prevAppointmentOptions = json.data.map((d) => {
				...
				const date = dayjs(new Date(d.date)).hour(splitTime[0]).minute(splitTime[1]);
				return {
					value: date.toDate(),
...
<svelte:head>
	<title>歌斐木診所 - {$t('ivc.title')}</title>
	<meta name="description" content={`歌斐木診所 - ${$t('ivc.description')}`} />
	<meta property="og:type" content="website" />
	<meta property="og:title" content={$t('clinic.fullName')} />
	<meta property="og:description" content={$t('clinic.description')} />
...
<h1 class={styles.title}>{$t('ivc.title')}</h1>
<!-- <Tabs items={tabItems} bind:selectedTab {onTabClick} /> -->
<form>
	<Tabs>
		<TabItem open title={$t('ivc.options.newAppointment')}>
			<div class={styles.formGroup}>
				<Label for="fullName">{$t('ivc.field.fullName.label')}</Label>
				<Input id="fullName" bind:value={fullName} on:input={updateFormState} required />
			</div>
			<div class={styles.formGroup}>
				<Label for="mobile">{$t('ivc.field.mobile.label')}</Label>
				<Input
					id="mobile"
					bind:value={mobile}
					type="tel"
			...
			<div class={styles.formGroup} class:dn={hideNewOptions}>
				<Label for="date">{$t('ivc.field.date.label')}</Label>
				<Select
					id="date"
					items={dateOptions}
					isSearchable={false}
			...
			<div class={styles.formGroup} class:dn={hideNewOptions}>
				<Label for="time">{$t('ivc.field.time.label')}</Label>
				<Select
					id="time"
					items={timeOptions}
					{groupBy}
		...
		<TabItem title={$t('ivc.options.modifyAppointment')}>
				...
				{#each editTypes as editType}
					<label class={styles.radioButton}>
						<input
							...
							value={editType}
						/>
						{$t(`ivc.editOptions.${editType}`)}
					</label>
				{/each}
			</div>
			<div class={styles.formGroup} class:dn={hidePrevAppointments}>
				<span>{$t('ivc.field.prevAppointment')}</span>
			</div>
			<div class={styles.formGroup} class:dn={hideNewOptions}>
				<span>{$t('ivc.field.newOptions')}</span>
			</div>
			<div class={styles.formGroup} class:dn={hidePrevAppointments}>
				<label for="prevAppointment">{$t('ivc.field.prevAppointment.label')}</label>
				<Select
					id="prevAppointment"
...
<style lang="less">
	...
	.themed .selectContainer > input::-moz-placeholder {
		font-family: Taipei Sans, huninn, Roboto, -apple-system, Segoe UI, Ubuntu, Fira Sans,
